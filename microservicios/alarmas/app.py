...
async def error_no_controlado(_request, _exc: Exception) -> JSONResponse:
    return JSONResponse(
        status_code=500, content={"ok": False, "error": "Error interno del servicio"}
    )
...
async def error_http(_request: Request, exc: HTTPException) -> JSONResponse:
    return JSONResponse(
        status_code=exc.status_code, content={"ok": False, "error": str(exc.detail)}
    )
...
async def error_validacion(
    ...
    return JSONResponse(
        status_code=422, content={"ok": False, "error": "Parametros invalidos"}
    )
...
def listar_alarmas(
    ...
    return ApiResponse(
        data=alarmas_service.obtener_alarmas(
            tipo=tipo,
            estado=estado,
            q=q,
            limit=limit,
